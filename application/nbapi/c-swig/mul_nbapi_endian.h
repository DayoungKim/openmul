...
static inline void nbapi_endian_convert_c_sw_port(struct c_sw_port *port, uint32_t (*convl)(uint32_t)) {
...
static inline void ntoh_c_sw_port(struct c_sw_port *port) {
    nbapi_endian_convert_c_sw_port(port, ntohl);
}

static inline void hton_c_sw_port(struct c_sw_port *port) {
...
static inline void nbapi_endian_convert_ofp_header(struct ofp_header *header, uint16_t (*convs)(uint16_t) UNUSED, uint32_t (*convl)(uint32_t)) {
...
static inline void nbapi_endian_convert_c_ofp_switch_add(struct c_ofp_switch_add *switch_info, uint16_t (*convs)(uint16_t), uint32_t (*convl)(uint32_t), uint64_t (*convll)(uint64_t)) {
...
static inline void ntoh_c_ofp_switch_add(struct c_ofp_switch_add *switch_info){
    nbapi_endian_convert_c_ofp_switch_add(switch_info, ntohs, ntohl, ntohll);
}

static inline void hton_c_ofp_switch_add(struct c_ofp_switch_add *switch_info){
...
static inline void nbapi_endian_convert_flow(struct flow *fl, uint16_t (*convs)(uint16_t), uint32_t (*convl)(uint32_t), uint64_t (*convll)(uint64_t)) {
...
static inline void
nbapi_endian_convert_c_ofp_flow_info(struct c_ofp_flow_info *cofp_fi, uint16_t (*convs)(uint16_t), uint32_t (*convl)(uint32_t), uint64_t (*convll)(uint64_t))
{
...
static inline void nbapi_endian_convert_c_ofp_host_mod(struct c_ofp_host_mod *cofp_host, uint16_t (*convs)(uint16_t), uint32_t (*convl)(uint32_t), uint64_t (*convll)(uint64_t)) {
...
static inline void ntoh_c_ofp_flow_info(struct c_ofp_flow_info *cofp_fi) {
	nbapi_endian_convert_c_ofp_flow_info(cofp_fi, ntohs, ntohl, ntohll);
}


static inline void ntoh_c_ofp_fabric_host(struct c_ofp_host_mod *cofp_host) {
	nbapi_endian_convert_c_ofp_host_mod(cofp_host, ntohs, ntohl, ntohll);
}

static inline void nbapi_endian_convert_c_ofp_group_mod(struct c_ofp_group_mod *cofp_gm, uint32_t (*convl)(uint32_t), uint64_t (*convll)(uint64_t)) {
...
static inline void ntoh_c_ofp_group_mod(struct c_ofp_group_mod *cofp_gm){
    nbapi_endian_convert_c_ofp_group_mod(cofp_gm, ntohl, ntohll);
}

static inline void nbapi_endian_convert_c_ofp_bkt(struct c_ofp_bkt *bkt, uint16_t (*convs)(uint16_t), uint32_t (*convl)(uint32_t)) {
...
static inline void ntoh_c_ofp_bkt(struct c_ofp_bkt *bkt) {
...
static inline void nbapi_endian_convert_ofp_action_header(struct ofp_action_header *header, uint16_t(*convs)(uint16_t)) {
    header->type = convs(header->type);
    header->len = convs(header->len);
}

static inline void ntoh_ofp_action_header(struct ofp_action_header *header) {
    nbapi_endian_convert_ofp_action_header(header, ntohs);
}

static inline void nbapi_endian_convert_nbapi_flow_brief(struct c_ofp_flow_info *cofp_fi, uint16_t(*convs)(uint16_t),uint32_t(*convl)(uint32_t), uint64_t(*convll)(uint64_t))
{
    //ntoh_ofp_header(&(cofp_fi->header));
    //ntoh_flow(&(cofp_fi->flow));
    nbapi_endian_convert_ofp_header(&(cofp_fi->header), convs, convl);
    nbapi_endian_convert_flow(&(cofp_fi->flow), convs, convl, convll);
  //  nbapi_endian_convert_flow(&(cofp_fi->mask), convs, convl, convll);
    cofp_fi->datapath_id = convll(cofp_fi->datapath_id);
...
static inline void ntoh_nbapi_flow_brief(struct c_ofp_flow_info *cofp_fi)
{
    nbapi_endian_convert_nbapi_flow_brief(cofp_fi, ntohs, ntohl, ntohll);
}

static inline void ntoh_ofp140_port_stats(struct ofp140_port_stats *ofp_ps){
...
static inline void nbapi_endian_convert_ofp131_port_stats(struct ofp131_port_stats *ofp_ps, uint32_t(*convl)(uint32_t), uint64_t(*convll)(uint64_t))
{
...
static inline void ntoh_ofp131_port_stats(struct ofp131_port_stats *ofp_ps){
    nbapi_endian_convert_ofp131_port_stats(ofp_ps, ntohl, ntohll);
}
static inline void nbapi_endian_convert_ofp_port_stats(struct ofp_port_stats *ofp_ps, uint16_t(*convs)(uint16_t), uint64_t(*convll)(uint64_t)){
...
static inline void nbapi_endian_convert_ofp_group_features(struct ofp_group_features *ofp_gf, uint32_t(*convl)(uint32_t))
{
...
static inline void nbapi_endian_convert_ofp_meter_features(struct ofp_meter_features *ofp_mf, uint32_t(*convl)(uint32_t))
{
...
static inline void ntoh_ofp_group_features(struct ofp_group_features *ofp_gf){
    nbapi_endian_convert_ofp_group_features(ofp_gf, ntohl);
}
static inline void ntoh_ofp_meter_features(struct ofp_meter_features *ofp_mf)
{

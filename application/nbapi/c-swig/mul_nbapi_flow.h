...
//int add_static_flow(uint64_t datapath_id, struct flow *fl, struct flow *mask, uint8_t priority, mul_act_mdata_t *mdata, uint64_t flags, int drop);
...
void nbapi_group_action_add(int act_len, struct of_group_mod_params *g_parms, mul_act_mdata_t *mdata, char *weight, char *ff_port, char *ff_group);
...
char *nbapi_parse_nw_addr_to_str(struct flow *flow, struct flow *mask, char *s);//(uint32_t nw_addr)
...
struct flow *nbapi_make_flow(char *smac, char *dmac, char *eth_type, char *vid, char *vlan_pcp, char *mpls_label, char *mpls_tc, char *mpls_bos, char *dip, char *sip, char *proto, char *tos, char *dport, char *sport, char *inport, char *table);
struct flow *nbapi_make_mask(char *smac, char *dmac, char *eth_type, char *vid, char *vlan_pcp, char *mpls_label, char *mpls_tc, char *mpls_bos, char *dip, char *sip, char *proto, char *tos, char *dport, char *sport, char *inport);
...
//void nbapi_mdata_input_actions(mul_act_mdata_t* mdata, GSList* actions);
...
struct ofp_action_set_field *nbapi_make_action_set_tp_port(uint8_t ip_proto, bool is_src, uint16_t port);
...
nbapi_switch_flow_list_t get_single_flow(uint64_t datapath_id, struct flow *flow, struct flow *mask, uint32_t prio);

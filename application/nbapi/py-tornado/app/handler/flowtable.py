    ...
    def post(self, dpid, flow_id=None):
        ...
        try:
            ...
            if check == 0:
                ...
                ret.update({'flow_id':str(check)})
        except Exception, e:
            ret.update({"error_message" : "failed to add flow!", "reason" : str(e)})
        finally:
            #end = datetime.datetime.now()
            #logger.debug("%10f"%((end-start).total_seconds()*1000)+" milliseconds")
            self.finish(ret)
...
#            self.__inst_action_check(version, body)
            nw_src = str(body['nw_src']) if body['dl_type'] and int(body['dl_type'], 16) != 0x86dd else str(body['nw_src6'])
            nw_dst = str(body['nw_dst']) if body['dl_type'] and int(body['dl_type'], 16) != 0x86dd else str(body['nw_dst6'])
            ...
            logger.debug("%.10f"%float((datetime.datetime.now()-start).total_seconds()*1000)+" milliseconds")
            self.finish(ret)
    '''
